#!/usr/bin/env python3
"""
maharashtra_data_prep.py  --  v3
=================================
Reads the Maharashtra GeoJSON road-segment data and combines it with
...
------
  ADB_Innovation_Maharashtra.geojson
      ~14,082 road segments.  Key columns used here:
  ...
  Road_Safety_Performance_Indicators_(Helmet_Wearing_results).xlsx
      ...
      Years present: 2022, 2023, 2024, 2025, plus "All" (multi-year aggregate).
...
-----------------------------------
  ...
  HelmetSPI_2022..2025  -- national Combined rate per survey year (broadcast)
...
OUTPUTS  -->  results/Analysis/data preparation/Maharashtra/v3/
---------------------------------------------------------------
  maharashtra_combined.csv       All 14,082 segments with SPI columns (no geometry)
  maharashtra_combined.parquet   Same table WITH geometry preserved (for QGIS / spatial joins)
...
# Force UTF-8 output so Unicode characters do not raise UnicodeEncodeError
# on a Windows console (which defaults to cp1252 encoding).
...
# PATHS
# =============================================================================
# __file__ resolves to: .../script/data preparation/Maharashtra/v3/maharashtra_data_prep.py
# Four dirname() calls walk up to the project root:
#   v3/  ->  Maharashtra/  ->  data preparation/  ->  script/  ->  project root
...
REGION = "Maharashtra"   # must match the "Location" column value in the Excel file
...
preview_cols = ["OBJECTID", "DISSOLVE_ID", "RoadClass", "LandUse",
...
# We filter to Maharashtra only and inspect what years are available.
#
# Year = "All" is the multi-year aggregate -- the most stable SPI value
# because it is not tied to a single survey cycle.
# Individual years (2022-2025) capture yearly variation.
# =============================================================================
print("\n" + "=" * 60)
...
# Filter to Maharashtra rows only.  Other locations (Thailand, Mumbai, Pune)
...
# Save the full Maharashtra SPI table as a reference file so analysts can see
...
#   Columns: HelmetSPI_2022 | HelmetSPI_2023 | HelmetSPI_2024 | HelmetSPI_2025
...
spi_year_cols      = []    # list of column names added, e.g. ["HelmetSPI_2022", ...]
...
#
# Join 1 (lookup join):
#   Left table  : gdf  (all 14,082 segments)
...
#                 Segments without a LandUse value receive NaN for SPI columns.
#
# Join 2 (broadcast):
#   No merge operation needed -- each per-year value is assigned directly
#   as a constant scalar column.  All 14,082 rows get the same value per year.
#
...
csv_path = os.path.join(RESULTS_DIR, "maharashtra_combined.csv")
gdf.drop(columns="geometry").to_csv(csv_path, index=False)
print(f"  [Saved] maharashtra_combined.csv")
...
parquet_path = os.path.join(RESULTS_DIR, "maharashtra_combined.parquet")
gdf.to_parquet(parquet_path, index=False)
print(f"\n  [Saved] maharashtra_combined.parquet")
