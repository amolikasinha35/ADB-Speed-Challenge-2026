#!/usr/bin/env python3
"""
phase_d_final.py  --  Phase D: Final Scoring & Deliverables  --  Maharashtra  --  v2
======================================================================================
...
-----
  ...
  D3  Generate three spatial outputs (segment map, aggregate choropleth, top-100 CSV).
  D4  Country comparison using helmet trend data from both regions.

Outputs  -->  results/Analysis/Analysisv2/Maharashtra/v2/
---------------------------------------------------------
  speed_safety_scores_maharashtra.gpkg    Final scored sections
  speed_safety_scores_maharashtra.parquet
  priority_top100_maharashtra.csv         Top 100 with Power Model benefits
  map_segment_maharashtra.html            Segment-level interactive map
  map_aggregate_maharashtra.html          Zone-level choropleth
  country_comparison.csv                  Maharashtra vs Thailand summary
...
subprocess.check_call(
    [sys.executable, "-m", "pip", "install",
     "geopandas", "pandas", "numpy", "pyarrow", "folium", "branca", "openpyxl", "fiona"],
...
enriched_path = os.path.join(RESULTS_DIR, "sections_enriched_maharashtra.parquet")
scored_path   = os.path.join(RESULTS_DIR, "sections_scored_maharashtra.parquet")
...
# Use adjusted benchmark if available (from Phase C); else base benchmark.
...
# VRU weight: refined by VLM vru_activity if available.
vru_weight = np.where(sections["LandUse"].str.upper() == "URBAN",
                      VRU_WEIGHT_URBAN, VRU_WEIGHT_RURAL)
if "vru_activity" in sections.columns:
    # Upgrade VRU weight if VLM observed frequent VRU activity.
    vru_weight = np.where(sections["vru_activity"] == "frequent",
                          vru_weight * 1.3, vru_weight)
sections["final_exposure"] = 1 + (1 - sections["HelmetSPI"].fillna(
    sections["HelmetSPI"].mean())) * vru_weight

# Consequence = 2 - forgivingness  (range 1.0–2.0; Phase B default = 1.5)
forgiveness = sections.get("forgivingness_index",
                            pd.Series(0.5, index=sections.index))
...
# Signal A: how far is the posted limit above the Safe System benchmark?
#   Positive = limit is too high.
sections["signal_A"] = (
    sections["SpeedLimit_num"] -
    sections.get("adjusted_benchmark_kmh", sections["base_benchmark_kmh"])
)
# Signal B: how far does V85 exceed the posted limit?
#   Positive = traffic ignores the limit.
sections["signal_B"] = (
    sections["F85thPercentileSpeed"] - sections["SpeedLimit_num"]
)

visual_char = sections.get("visual_speed_character", pd.Series("", index=sections.index))
...
# Target iRAP star uplift by intervention.
STAR_UPLIFT = {
...
cmap_score = cm.LinearColormap(
    ...
    vmin=score_log.quantile(0.05), vmax=score_log.quantile(0.95),
...
sub = map_data[["section_id", "final_score_log", "final_score",
                "MedianSpeed", "SpeedLimit_num", "base_benchmark_kmh",
                "intervention_type", "final_score_band", "geometry"]].dropna(subset=["final_score_log"]).copy()
...
folium.GeoJson(
    ...
    tooltip=folium.GeoJsonTooltip(
        fields=["section_id", "final_score", "MedianSpeed", "SpeedLimit_num",
                "base_benchmark_kmh", "intervention_type"],
        aliases=["Section", "Score", "Median Speed", "Posted Limit",
...
map1_path = os.path.join(RESULTS_DIR, "map_segment_maharashtra.html")
m1.save(map1_path)
print(f"  [Saved] map_segment_maharashtra.html")

# ── Output 2: Aggregate zone choropleth (Boundaries_4helmet polygons) ────────
try:
    import fiona
    helmet_zones = gpd.read_file(GPKG_PATH, layer=GPKG_HELM_LAYER)
    if helmet_zones.crs != sections.crs:
        helmet_zones = helmet_zones.to_crs(sections.crs)

    # Determine zone name column (use first text column that isn't the SPI cols).
    name_candidates = [c for c in helmet_zones.columns
                       if c not in (SPI_ALL_COL, SPI_DRV_COL, SPI_PASS_COL, "geometry")
                       and helmet_zones[c].dtype == object]
    zone_name_col = name_candidates[0] if name_candidates else "zone"
    if zone_name_col not in helmet_zones.columns:
        helmet_zones[zone_name_col] = [f"Zone_{i}" for i in range(len(helmet_zones))]

    # Spatial join: section centroids → helmet zone.
    sec_pts = sections.copy()
    sec_pts.geometry = sec_pts.geometry.centroid
    sec_joined = gpd.sjoin(
        sec_pts[["section_id", "final_score", "HelmetSPI",
                 "final_score_band", "geometry"]],
        helmet_zones[[zone_name_col, "geometry"]],
        how="left", predicate="within",
    ).drop_duplicates(subset="section_id")

    zone_agg = sec_joined.groupby(zone_name_col).agg(
    ...
    zone_csv = os.path.join(RESULTS_DIR, "zone_aggregate_maharashtra.csv")
    zone_agg.to_csv(zone_csv, index=False)
    print(f"  [Saved] zone_aggregate_maharashtra.csv")

    zone_geo = helmet_zones[[zone_name_col, "geometry"]].merge(
        zone_agg, on=zone_name_col, how="left"
    )
    zone_geo["mean_score"] = zone_geo["mean_score"].fillna(0)

    m2 = folium.Map(location=map_center, zoom_start=ZOOM, tiles="CartoDB positron")
    score_cmap2 = cm.LinearColormap(
        colors=["#1a9850", "#fee08b", "#d73027"],
        vmin=zone_geo["mean_score"].min(),
        vmax=zone_geo["mean_score"].max(),
        caption="Mean Speed Safety Score by Zone",
    )
    ...
    folium.GeoJson(
        ...
        tooltip=folium.GeoJsonTooltip(
            fields=[zone_name_col, "mean_score", "mean_helmet_spi",
                    "n_sections", "pct_critical"],
            aliases=["Zone", "Mean Score", "Mean Helmet SPI",
                     "Sections", "% Critical"],
            sticky=True,
        ),
    ).add_to(m2)
    ...
    map2_path = os.path.join(RESULTS_DIR, "map_aggregate_maharashtra.html")
    m2.save(map2_path)
    print(f"  [Saved] map_aggregate_maharashtra.html")

except Exception as e:
    print(f"  [WARN] Zone choropleth skipped: {e}")
    # Fallback: aggregate by LandUse column only.
    zone_agg = map_data.groupby("LandUse").agg(
        mean_score      = ("final_score",      "mean"),
        mean_helmet_spi = ("HelmetSPI",         "mean"),
        n_sections      = ("section_id",         "count"),
        pct_critical    = ("final_score_band",
    ...
    zone_csv = os.path.join(RESULTS_DIR, "zone_aggregate_maharashtra.csv")
    zone_agg.to_csv(zone_csv, index=False)
    print(f"  [Saved] zone_aggregate_maharashtra.csv  (fallback — no polygon map)")

# ── Output 3: Top-100 priority CSV with Power Model benefits ──────────────────
def power_model_reduction(row):
    """ΔFatal% = (1 - (V_post / V_op)^4) × 100"""
    ...
    if itype == "lower_limit":
        ...
        v_post = v_op
    reduction = (1 - (v_post / max(v_op, 1)) ** POWER_MODEL_EXPONENT) * 100
    return round(float(reduction), 2)

top100 = (
    sections.nlargest(TOP_N_PRIORITY, "final_score")
    .copy()
...
top100_path = os.path.join(RESULTS_DIR, "priority_top100_maharashtra.csv")
top100[OUT_COLS].to_csv(top100_path, index=False)
print(f"  [Saved] priority_top100_maharashtra.csv  ({len(top100)} sections)")
...
comparison_rows = []
for region_label in [REGION, "Thailand"]:
    ...
    if len(h_yr) >= 2:
        ...
        vulnerability_index = latest_spi * (1 + abs(float(slope)) * 5)
    else:
        slope, latest_spi, vulnerability_index = 0.0, h_yr["SPI"].mean() if len(h_yr) else 0.5, 0.5

    # Load the other region's scores if available.
    if region_label == REGION:
        mean_score = float(sections["final_score"].mean())
        n_sections = len(sections)
    else:
        th_path = os.path.join(BASE_DIR, "results", "Analysis", "Analysisv2", "Thailand", "v2",
                               "speed_safety_scores_thailand.parquet")
        if os.path.exists(th_path):
            th = gpd.read_parquet(th_path)
            mean_score = float(th["final_score"].mean())
            n_sections = len(th)
        else:
...
sections.to_file(
    os.path.join(RESULTS_DIR, "speed_safety_scores_maharashtra.gpkg"),
    driver="GPKG", layer="speed_safety_scores",
)
print(f"  [Saved] speed_safety_scores_maharashtra.gpkg  ({len(sections):,} sections)")

sections.to_parquet(
    os.path.join(RESULTS_DIR, "speed_safety_scores_maharashtra.parquet"), index=False
)
print(f"  [Saved] speed_safety_scores_maharashtra.parquet")

with open(os.path.join(RESULTS_DIR, "phase_d_manifest.json"), "w") as f:
    json.dump({
        "region":           REGION, "phase": "D",
