#!/usr/bin/env python3
"""
phase_c_vlm.py  --  Phase C: VLM Enrichment  --  Maharashtra  --  v2
======================================================================
...
---------------------------------------------------------
  ...
  gsv_image_locations_maharashtra.csv   Image name, Lat, Long
  ...
  sections_enriched_maharashtra.gpkg
  sections_enriched_maharashtra.parquet
...
gsv_loc_df.to_csv(os.path.join(RESULTS_DIR, "gsv_image_locations_maharashtra.csv"), index=False)
...
print(f"  [Saved] imagery_metadata.csv  "
      f"|  gsv_image_locations_maharashtra.csv  ({len(gsv_loc_df):,} rows)")
...
def _gemini_with_retry(img, max_retries=4):
    for attempt in range(max_retries):
        try:
            return model.generate_content([VLM_PROMPT, img]).text
...
sections = gpd.read_parquet(
    os.path.join(RESULTS_DIR, "sections_scored_maharashtra.parquet")
)
...
sections.to_file(
    os.path.join(RESULTS_DIR, "sections_enriched_maharashtra.gpkg"),
    driver="GPKG", layer="sections_enriched",
)
print(f"  [Saved] sections_enriched_maharashtra.gpkg  ({len(sections):,} sections)")

sections.to_parquet(
    os.path.join(RESULTS_DIR, "sections_enriched_maharashtra.parquet"), index=False
)
print(f"  [Saved] sections_enriched_maharashtra.parquet")
